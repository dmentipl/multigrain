...
\begin{abstract}
   ...
   large Stokes number dust grains, typical of evolved protoplanetary discs and
   debris discs. The method improves upon earlier methods, in which only a
   single grain size could be represented, by capturing the backreaction of
   multiple dust species on the gas. This effect is greater for large
   dust-to-gas ratios that may be expected in the later stages of the
   protoplanetary disc life.
\end{abstract}
...
\end{align}
%
where \(\rho_j\) and \(\vec{v}_j\) are the density and velocity of the fluids,
\(P\) is the gas pressure, \(\vec{f}\) is any body forces acting on the fluids,
and \(K_i\) is the drag coefficient between the gas and a particular dust
species, \(i\). Note that each dust fluid has one gas drag interaction term,
whereas the gas momentum equation has a sum of interactions over each dust
...
\end{align}
%
% We can now define a stopping-time-like time scale, \(t_{s_i} = \rho / K_i\)
...
For ``two-fluid'' see \citet{Laibe2012MNRAS.420.2345L}, Section 2.2.
