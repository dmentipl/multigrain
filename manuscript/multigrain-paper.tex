...
code repository.}. We then performed several tests to validate the method
against known analytical solutions. The dusty box test
...
The tests all have analytical solutions in one dimension. We peformed each of
the tests in three dimensions, and then reduced the data to one dimension for
comparison with the analytical solutions. In each of the tests we used, unless
stated otherwise: a Courant factor of 0.3 (\texttt{C\_cour} in
\textsc{Phantom}); a time step constraint on the acceleration \( C_{\rm force}
\), where \( \Delta t_a < C_{\rm force} \sqrt{h_a / |\vec{a}_a|} \), of 0.25
(\texttt{C\_force} in \textsc{Phantom}).
