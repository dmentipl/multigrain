...
\begin{itemize}
   ...
   \item \textsc{Phantom} dust methods---\citet{Price2018PASA...35...31P}.
   \item \textsc{Phantom} 1-fluid multigrain---\citet{Hutchison2018MNRAS.476.2186H}.
...
\textcolor{red}{We implemented the above scheme in \textsc{Phantom}... Each test
   is performed in 3d. Quintic kernel. hfact value.}
...
\begin{figure*}
   \begin{center}
      ...
      \caption{Dusty box numerical test showing the differential velocity
         ...
         \textsc{Phantom} simulation. The solid and dashed lines represent the
         analytical solution with and without taking back reaction into account,
...
the \textsc{Phantom} simulation agrees. For a larger dust-to-gas ratio (0.5) the
analytical solutions diverge, and the \textsc{Phantom} simulation data follows
the backreaction-inclusive solution. For large dust-to-gas ratio we see that the
...
length, with 8192 for each species (128 particles in the wave direction). We use
...
numerical solution from \textsc{Phantom}. We first note that the numerical
solution accurately reproduces the analytical solution. In all cases we see the
wave damping effect.
...
1.0, 1.2, 1.5, 1.8 \), corresponding to \(\overline{N}_{\rm neigh} = 113, 195,
381, 660\). We can see that the accuracy of the gas density is independent (or
only weakly dependent) on the neighbour number. In contrast, the dust density
depends strongly on the neighbour number. We require \( h_{\rm fact} = 1.5\),
for the single dust species case, and \(h_{\rm fact} = 1.8\), for the three dust
species case.
...
\begin{figure*}
   \begin{center}
      \includegraphics[width=0.66\textwidth]{figs/dustyshock_hfact.pdf}
      \caption{\label{fig:dustyshock_hfact}}
   \end{center}
\end{figure*}
...
\begin{itemize}
   ...
   \item Dustyshock hfact comparison; see Figure~\ref{fig:dustyshock_hfact}.
\end{itemize}
}
...
visualisation of \textsc{Phantom} data \citep{Mentiplay2019JOSS....4.1884M}.
