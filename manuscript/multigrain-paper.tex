...
dust-gas mixtures with multiple dust species with both grid and particle methods
\citep{Bai2010ApJS..190..297B, Porth2014ApJS..214....4P,
...
Laibe2014MNRAS.440.2136L, Price2015MNRAS.451..813P}. In this approach the dust
fraction is stored on the particles and evolved in time.
...
is computationally inefficient. Various groups have developed semi-implicit
timestepping methods which remove the restriction of small timesteps required by
explicit schemes for drag \citep{Monaghan1997JCoPh.138..801M,
Laibe2012MNRAS.420.2365L, Loren-Aguilar2014MNRAS.443..927L,
Loren-Aguilar2015MNRAS.454.4114L, Stoyanovskaya2018A&C....25...25S}.
...
weakly-coupled grains \citep{Laibe2014MNRAS.440.2147L}.
...
\end{align}
%
Even though using a pre-factor in Equation~\ref{eqn:sph-timestep-constraint} of
2 provides an numerically stable timestepping scheme (in the von Neumann sense),
numerical experimentation showed that any value greater than 1.0 led to
inaccurate results. We choose a value of 0.9.

For a dust particle, we take the minimum over gas neighbours of the individual
species constraint (\(\Delta t < 0.9 t_{s_i}\)):
%
\begin{align}
...
close-packed lattice, using dense-sphere packing, such that there were 648 gas
particles (8 particles in the direction of motion), and 648 dust particles per
species (8 particles in the direction of motion). We set the gas density to
\(10^{-13}\)~g~cm\({}^{-3}\), with the dust-to-gas ratio varying per test
problem. We set the dust grain material density for all species to
1.0~g~cm\({}^{-3}\).
