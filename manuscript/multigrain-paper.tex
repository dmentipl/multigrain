...
spatial dimensions, \(m^k_a\) and \(h^k_a\) are the SPH particle mass and
smoothing length, respectively, \(W_{ab}\) is the SPH smoothing kernel and
\(\eta\) is a factor of order unity which determines the number of neighbours
per particle. For an overview on the SPH method, see:
...
the equations of motion for the gas and each dust species is
%
\begin{align}
   \frac{\dd \vec{v}^k_a}{\dd t} = - \sum_b m^k_b \left[
      \frac{P^k_a}{\Omega^k_a \left(\rho^k_a\right)^2} \nabla_a W_{ab}(h^k_a) +
      \frac{P^k_b}{\Omega^k_b \left(\rho^k_b\right)^2} \nabla_a W_{ab}(h^k_b)
   \right],
\end{align}
%
for \(k \in \{g, d_1, \dots, d_N\}\).
