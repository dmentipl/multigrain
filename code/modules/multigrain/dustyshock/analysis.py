...
def plot_quantity_profile_subsnaps(snap, quantity, axs, xrange):
...
def plot_velocity_density(*, snap, xrange, fig_kwargs={}):
    fig, axs = make_fig_axs(snap=snap, ncols=2, **fig_kwargs)
    plot_quantity_profile_subsnaps(
        snap=snap, quantity='velocity_x', axs=axs[:, 0], xrange=xrange
    )
    plot_quantity_profile_subsnaps(
        snap=snap, quantity='density', axs=axs[:, 1], xrange=xrange
    )
    return fig
