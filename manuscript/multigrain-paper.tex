...
\textcolor{red}{
   We implemented the above scheme in Phantom... Each test is performed in 3d.
   Quintic kernel. hfact value.
}
...
\begin{figure*}
   \begin{center}
      ...
      \caption{Dusty shock numerical test showing the velocity (top) and the
         ...
         species. The open circles represent the results from the Phantom
         simulation. The solid lines represent the analytical solution from
...
\textcolor{red}{(Should this figure be of the particles?)} We set up the
particles on two close-packed lattices, one on each side of a discontinuity in
the density and velocity in the x-direction with values on either side of the
shock set by their asymptotic values. Due to the resolution-follows-the-mass
nature of SPH, the numerical resolution is higher in the high density region.
\textcolor{red}{Add number of particles.} We smoothed the density distribution,
but not the velocity, using a logistic function, \(f(x) \propto {\left(1 +
e^{-k(x-x_{\rm shock})} \right)}^{-1}\). The large (infinite) gradient in
density at the shock boundary leads to errors in evolving the dust.

\textcolor{red}{discussion of SPH parameters---hfact; dust same as gas}
...
position to minimise the error.
...
We used Phantom to perform the numerical simulations
\citep{Price2018PASA...35...31P}. We used Plonk for analysis and visualisation
of Phantom data \citep{Mentiplay2019JOSS....4.1884M}.
