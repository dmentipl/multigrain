...
shock set by their asymptotic values. Due to errors in evolving the dust derived
from the (infinite) gradient at the shock boundary we smooth the density
distribution using a logistic function, \(f(x) \propto {\left(1 + e^{-k(x-x_{\rm
shock})} \right)}^{-1}\).

\textcolor{red}{
   \begin{itemize}
      \item discussion of SPH parameters---hfact, nx
   \end{itemize}
}
...
We used Plonk for analysis and visualisation of Phantom data
\citep{Mentiplay2019JOSS....4.1884M}.
