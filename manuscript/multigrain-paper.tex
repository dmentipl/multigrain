...
\textcolor{red}{
\begin{itemize}
   \item We use explicit time stepping. Write up algorithm.
   \item Drag timescale limits calculation to \(\mathrm{St} \gtrsim 1\):
      \begin{align}
         \quad\quad\quad \Delta t_{\mathrm{drag}}^a = \min_i \min_b t_{\mathrm{s},i}^{ab}
      \end{align}
...
\(k\) is the wavenumber and the \(V_j\) terms depend only on time, for both the
gas and all dust species. This expansion leads to the following matrix equation:
%
\begin{align}
   \begin{pmatrix}
...
\frac{\Delta t K_i}{\rho_{d_i}} - \lambda \right) \right]^{-1}\) is second order
in \(\Delta t\), so we neglect it. This gives the eigenvalues as
%
\begin{align}
